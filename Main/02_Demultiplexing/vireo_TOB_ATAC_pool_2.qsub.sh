...
#PBS -P fy54
#PBS -q normalbw
#PBS -N vireo_demultiplex_TOB_ATAC_pool_2
...
folders=($(find ${DATA_DIR} -maxdepth 2 -type d -name "pool_2*" -exec readlink -f {} \;))
# Check if the folders array is empty
if [ ${#folders[@]} -eq 0 ]; then
    folders=($(find "${DATA_DIR}" -maxdepth 2 -type d -name "pool_R_2" -exec readlink -f {} \;))
fi
echo ${folders}
...
VIREO_OUTDIR=/g/data/ei56/ax3061/proj/tenk10k/caQTL/demultiplexing/output/Vireo/pool_2
