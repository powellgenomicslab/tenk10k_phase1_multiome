...
RunEpiTraceAge <- function(epitrace_object,parallel=F,ncores=4,subsamplesize=2000,normalization_method='randomized',select_minimal_percentage = 0.05, select_size_of_dispersion = 3000){
  ...
  lapply(names(mtx_list),function(x){
    ...
    if(sum(Matrix::rowSums(testmtx)>0,na.rm=T)==0 & sum(Matrix::colSums(testmtx)>0,na.rm=T)==0){
      ...
      if(parallel==F){
        ...
        EpiTraceAge(mat = testmtx[Matrix::rowSums(testmtx)>0,Matrix::colSums(testmtx)>0],ncores = 4,parallel=T,subsamplesize=subsamplesize,normalization_method=norm_meth,select_minimal_percentage=select_minimal_percentage,select_size_of_dispersion=select_size_of_dispersion)
      }
    }
  }) -> res_list
...
EpiTraceAge <- function(mat,ncores=4,parallel=T,size=2000,batch=NULL,subsamplesize=2000,normalization_method='randomized',select_minimal_percentage = 0.05, select_size_of_dispersion = 3000){
  ...
  if(parallel==T){
    batches <- parallel::mclapply(subsamples, mc.cores = min(chunk, 4), function(subsample_x){
  ...
  if(parallel==T){
    epitrace_ranked <- parallel::mclapply(1:length(censored_cor_normalized_variable_mat_list), mc.cores = min(chunk,4),function(i) {
      rank_function(i)
    }) %>% unlist()
  }else{
...
AssociationOfPeaksToAge <- function(epitrace_object,peakSetName='peaks',epitrace_age_name='EpiTraceAge_all',subset_of_cells=NULL,epitrace_age_vector=NULL,parallel=F){
  ...
  if(parallel==F){
    ...
    parallel::mclapply(c(1:ceiling(dim(to_be_associated_mtx)[1]/1000)),mc.cores = 4,function(x){
      WGCNA::cor(x = t(to_be_associated_mtx[((1000*(x-1))+1):min(dim(to_be_associated_mtx)[1],1000*x),]), y = age_vec)
    }) -> res_list_cor
  }
...
EpiTraceAge_Convergence <- function(peakSet, matrix, celltype = NULL, min.cutoff = 50,lsi_dim = 2:50, fn.k.param = 21, ref_genome = "hg38", sep_string = c(":","-"), clock_gr = plyranges::reduce_ranges(c(clock_gr_list[[1]],clock_gr_list[[2]])), non_standard_clock = F, qualnum = 10,Z_cutoff = 3, mean_error_limit = 0.01, ncore_lim = 4, parallel = T,iterative_time = 2,remove_peaks_number=10,normalization_method='randomized',select_minimal_percentage = 0.05, select_size_of_dispersion = 3000){
  norm_meth = normalization_method
  ...
  epitrace_obj <- EpiTrace_prepare_object(initial_peakSet_clk,
                                          initial_matrix_clk, celltype, ref_genome = ref_genome, non_standard_clock = non_standard_clock,
  ...
  if (parallel == F) {
    ...
    epitrace_obj_age_estimated <- RunEpiTraceAge(epitrace_obj,
                                                 ncores = 4, parallel = T,normalization_method=norm_meth,select_minimal_percentage=select_minimal_percentage,select_size_of_dispersion=select_size_of_dispersion) %>% suppressMessages() %>%
      suppressWarnings()
  }
