...
for (celltype in celltype_list) {
    ...
    all_peaks <- readLines(glue(
        "/g/data/fy54/od8037/Brenner/SAIGEQTL/Phenotypes/FinalPeaks/{celltype}.txt"
    ))
    ...
    for (chr_num in 1:22) {
        ...
        peak_df <- data.table(peak_id = peaks) %>%
            ...
            dplyr::mutate(
                ...
                chr_num = chr_num,
                cis_start = centre_start - 100000,
                cis_end = centre_end + 100000
            ) %>%
            dplyr::mutate(
        ...
        dir.create(glue("/g/data/fy54/od8037/Brenner/SAIGEQTL/Rare/Regions/{celltype}"), showWarnings = FALSE)
        write.table(
            peak_df,
            file = glue("/g/data/fy54/od8037/Brenner/SAIGEQTL/Rare/Regions/{celltype}/chr{chr_num}.csv"),
            sep = "\t",
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE
