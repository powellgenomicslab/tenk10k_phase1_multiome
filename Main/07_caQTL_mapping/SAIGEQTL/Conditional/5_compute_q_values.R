...
for (celltype in celltype_list) {
    ...
    df <- as.data.frame(fread(glue(
        "/g/data/fy54/od8037/Brenner/SAIGEQTL/Rare/Conditional/FinalResults/Raw/{celltype}.csv"
    ), fill = TRUE)) %>% dplyr::filter(Group == "Cauchy")
    ...
    df$Pvalue_noACAT_cond <- mapply(
        function(p1, p2) get_CCT_pvalue(c(p1, p2)),
        df$Pvalue_Burden_cond, df$Pvalue_SKAT_cond
    )
    df$qvalue_cond <- qvalue(df$Pvalue_noACAT_cond)$qvalues
    ...
    final_df <- left_join(df, regions_df, by = "Region") %>%
        ...
        sort_peaks()
    fwrite(final_df, glue("/g/data/fy54/od8037/Brenner/SAIGEQTL/Rare/Conditional/FinalResults/Processed/{celltype}.csv"))
}
