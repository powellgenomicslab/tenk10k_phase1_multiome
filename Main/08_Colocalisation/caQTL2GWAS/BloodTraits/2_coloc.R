...
results_dir <- glue("/g/data/fy54/od8037/TenK10K/ColocNew/BloodTraits/Results/{condition}/{celltype}")
...
sig_result_df <- fread(glue(
    "/g/data/ei56/od8037/TenK10K/caQTLNew/ResultSummaries/{celltype}.csv"
))[, .(phenotype_id)]
...
sig_result_df[, chromosome := peak_coords[[1]]]
...
for (chr_num in 1:22) {
    ...
    sig_result_df_subset <- sig_result_df[chromosome == paste0("chr", chr_num)]
    num_caqtls <- nrow(sig_result_df_subset)
    print(paste0("Number of caQTLs for Chromosome ", chr_num, ": ", num_caqtls))
    if (num_caqtls == 0) {
    ...
    gwas_path <- glue("/g/data/fy54/od8037/TenK10K/ColocNew/BloodTraits/GWAS/{condition}/chr{chr_num}.fst")
    ...
    result_list <- vector("list", length(sig_result_df_subset$phenotype_id))
    for (i in seq_along(sig_result_df_subset$phenotype_id)) {
        peak <- sig_result_df_subset$phenotype_id[i]

        # Extract cis window coordinates
        cis_start <- sig_result_df_subset$cis_start[i]
        cis_end <- sig_result_df_subset$cis_end[i]
