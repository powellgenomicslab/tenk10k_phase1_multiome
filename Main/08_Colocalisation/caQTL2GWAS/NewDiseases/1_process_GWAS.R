...
for (condition_name in condition_list) {
    ...
    out_dir <- glue("/g/data/fy54/od8037/TenK10K/ColocNew/NewDiseases/GWAS/{condition_name}")
    ...
    df <- na.omit(df[, .(chr, beta, varbeta, position, snp)])

    # Save each chromosome separately
    for (chr_num in 1:22) {
        print(glue("Processing chromosome {chr_num} ..."))
        df_subset <- df[
            chr == chr_num, .(beta, varbeta, position, snp)
        ][!duplicated(snp)]
